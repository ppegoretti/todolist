...
//           const taskIndex = todos[editingTaskStatus].findIndex(t => t.id === Number.parseInt(editingTaskId));
//           if (taskIndex !== -1) {
  ...
  ;['todo', 'doing', 'done'].forEach((status) => {
    ...
    filteredTodos.forEach((todo) => {
      ...
      if (todo.subtasks && todo.subtasks.length > 0 && collapsedSubtasks.has(todo.id)) {
        ...
        const toggleButton =
          subtaskList.previousElementSibling.querySelector('.toggle-subtasks')
...
function validateTodosStructure(data) {
  ...
  if (!Array.isArray(data.todo) || !Array.isArray(data.doing) || !Array.isArray(data.done))
    return false
...
function handleSubtaskDrop(event) {
  ...
  const taskId = Number.parseInt(dropTarget.closest('.subtask-list').dataset.taskId)
  const status = dropTarget.closest('.list').dataset.status
  const task = todos[status].find((t) => t.id === taskId)

  if (!task || !task.subtasks) return

  const fromIndex = Number.parseInt(draggedSubtask.dataset.index)
  const toIndex = Number.parseInt(dropTarget.dataset.index)
...
function saveTask() {
  ...
  const subtaskInputs = document.querySelectorAll(
    '#subtasksList .subtask-list .subtask-input-group',
  )
  ...
  if (title && responsible && deadline) {
    if (isEditing && editingTaskId) {
      const taskIndex = todos[editingTaskStatus].findIndex(
        (t) => t.id === Number.parseInt(editingTaskId),
      )
      if (taskIndex !== -1) {
